...
class SearchViewModel @Inject constructor(
    ...
    fun onEvent(event: SearchEvent) {
        when(event) {
            ...
            is SearchEvent.OnAmountForFoodChange -> {
                state = state.copy(
                    detectableFood = state.detectableFood.map {
    ...
    private fun executeSearch() {
        viewModelScope.launch {
            state = state.copy(
                isSearching = true,
                detectableFood = emptyList()
            )
            marcadorUseCases
                .searchFood(state.query)
                .onSuccess { foods ->
                    state = state.copy(
                        detectableFood = foods.map {
                           DetectableFoodUiState(it)
                        },
                        isSearching = false,
                        query = ""
                    )
    ...
    private fun trackFood(event: SearchEvent.OnTrackFoodClick) {
        viewModelScope.launch {
            val uiState = state.detectableFood.find { it.food == event.food }
            marcadorUseCases.marcadorFood(
